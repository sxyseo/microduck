...
个人本地复刻工作台的阶段 0/1 最小实现。它管理项目、硬件档案、只读预检、训练编排和证据记录；不直接写舵机、不替代 `robotd`，也不会自动选择 HL-1910 或 HL-2915。
...
任务图还包含“装配与电气验收”“标定记录确认”“部署前兼容性预检”和“部署策略与健康检查”。兼容性运行依赖硬件确认、开发机预检和 smoke；真正生成部署计划时，还必须有当前硬件的装配检查全通过，以及带操作者明确核验的标定记录。部署执行会独立记录任务状态，只有上传、远端哈希、加载和健康检查都通过才算成功；它不代表站立或行走验收通过。硬件变更会同时使这些证据失效。
